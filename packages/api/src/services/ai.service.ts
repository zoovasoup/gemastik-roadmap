...
function classifyGeminiError(error: unknown) {
  ...
  if (message.includes('400') || message.includes('INVALID_ARGUMENT') || message.includes('Bad Request')) {
    return new GeminiServiceError(
      'bad_request',
  ...
  return new GeminiServiceError(
    'unknown',
    'Terjadi kegagalan saat menghubungi Gemini. Detail provider: ' + getCompactErrorDetails(error),
    error,
  )
}

async function callGeminiWithRetry(prompt: string, systemInstruction?: string, retries = 3): Promise<string> {
  let lastError: any
  ...
  for (let i = 0; i < retries; i++) {
    ...
    try {
      ...
      if (is503 && i < retries - 1) {
        console.warn(`[AI_RETRY] Attempt ${i + 1} failed (503). Retrying with ${modelName} in ${delay}ms...`)
        await sleep(delay)
...
export const aiService = {
  async generateText(prompt: string, systemInstruction?: string) {
    try {
      ...
      const classifiedError = classifyGeminiError(error)
      console.error('AI_SERVICE_TEXT_ERROR:', classifiedError.message, classifiedError.cause ?? error)
  ...
  async generateStructuredOutput(prompt: string, systemInstruction?: string) {
    try {
      ...
      const classifiedError = error instanceof GeminiServiceError ? error : classifyGeminiError(error)

      console.error('AI_SERVICE_STRUCTURED_ERROR:', classifiedError.message, classifiedError.cause ?? error)
      throw classifiedError
    }
  },
}
